...
public class MinionMenu {
    ...
    public MinionMenu(@NotNull org.bukkit.plugin.Plugin plugin, @NotNull MenuService menus,
                      ...
                      @NotNull LedgerService ledger, @Nullable SkylliaIntegration skyllia,
    ...
    MenuService.ClickHandler upgradeClick(@NotNull UUID minionId, @NotNull MenuService.Menu menu) {
        return (viewer, click) -> {
            ...
            if (skyllia != null) {
                Optional<IslandView> view = skyllia.islandOf(viewer.getUniqueId());
                if (view.isEmpty() || !view.get().islandId().equals(record.islandId())
                        || !view.get().canWithdraw()) {
                    viewer.sendMessage(miniMessage.deserialize(
            ...
            if (next.upgradeCostMoney() > 0) {
                ...
                ledger.authoritativeIslandBalance(record.islandId())
                        .whenComplete((balance, failure) -> playerThread(viewer, () -> {
                            if (failure != null || balance == null || balance < next.upgradeCostMoney()) {
                                upgrading.remove(minionId);
                                viewer.sendMessage(miniMessage.deserialize(
                                        "<red>Za mało monet na koncie wyspy ("
                                                + Ui.money(next.upgradeCostMoney()) + ").</red>"));
                                return;
                            }
                            // MINION-7: liczone raz przed asyncem, zdejmowane po — w międzyczasie
                            // gracz mógł wyrzucić składniki. Sprawdzamy ponownie i zdejmujemy
                            // PRZED wypłatą, bo składniki łatwiej oddać niż cofnąć przelew.
                            ...
                            ledger.withdrawIsland(record.islandId(), next.upgradeCostMoney(),
                                            "minion:upgrade:" + minionId + ":" + System.currentTimeMillis(),
                                            "minion_upgrade")
                                    .whenComplete((mutation, paymentFailure) -> playerThread(viewer, () -> {
                                        try {
                                            if (paymentFailure != null || mutation == null || !mutation.applied()) {
    ...
    MenuService.ClickHandler pickUpClick(@NotNull UUID minionId) {
        return (viewer, click) -> {
            ...
            if (skyllia != null) {
                Optional<IslandView> view = skyllia.islandOf(viewer.getUniqueId());
                if (view.isEmpty() || !view.get().islandId().equals(current.islandId())
                        || !view.get().canWithdraw()) {
                    viewer.sendMessage(miniMessage.deserialize(
