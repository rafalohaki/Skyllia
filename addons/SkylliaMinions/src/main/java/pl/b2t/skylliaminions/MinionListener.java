...
public final class MinionListener implements Listener {
    ...
    public void onMinionItemPlace(PlayerInteractEvent event) {
        ...
        Island playerIsland = Islands.islandOf(player.getUniqueId());
        Island atIsland = Islands.islandAt(base);
        if (playerIsland == null || atIsland == null
                || !playerIsland.getId().equals(atIsland.getId())) {
            player.sendMessage(miniMessage.deserialize(
        ...
        int extraSlots = extraSlots(player) + Math.max(0, islandExtraSlots.applyAsInt(islandId));
    ...
    public void onMinionInteract(PlayerInteractAtEntityEvent event) {
        ...
        Island my = Islands.islandOf(player.getUniqueId());
        if (my == null || !my.getId().equals(record.islandId())) {
            player.sendMessage(miniMessage.deserialize(
